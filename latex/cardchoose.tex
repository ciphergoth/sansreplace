...
universe \(U\) of cardinality \(k\), and \(\multichoose{|U|}{k} \defeq \left|\multichoose{U}{k}\right|\),
which is well defined since it depends only on the number of elements in \(U\).
...
there is a bijection between \(\multichoose{U}{k}\) and the set of 
cardinality \(k\) subsets of \(S\), from which we infer that 
$\multichoose{|U|}{k} = \binom{|U| + k - 1}{k}$ wherever \(|U| + k > 0\).

\section{Intuition behind the algorithm}

Every way of choosing 6 integers from the range $\NN_{<11}$ is equivalent to a way of
...
For example, $\multichoose{\NN_{<2}}{3} = \{[0, 0, 0], [0, 0, 1], [0, 1, 1], [1, 1, 1]\}$; for each of
