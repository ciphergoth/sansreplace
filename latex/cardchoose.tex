...
Someone who calls a routine that promises a fairly drawn \(k\)-element subset of
...
\begin{itemize}
    ...
    \item a set of integers in some kind of efficient set structure (eg a hash table).
\end{itemize}

Both Python's \mintinline{python}{random.sample()} and 
Rust's \mintinline{python}{rand::seq::index::sample} return in a fair random order.

A routine which returns one of these can straightforwardly be converted into 
a routine that returns another, using eg a sort or a Fisher-Yates shuffle as appropriate.
...
\begin{center}
    ...
    \hline
    Quadratic rejection sampling & Random & none & \(k^2\) \\
    Set-based rejection sampling & Random & Set & \(k\) \\
    Algorithm S & Sorted & none & \(n\) \\
    Algorithm R & Random & none & \(n\) \\
    SAMPLE & Random & \(n\)-element list & \(n\) \\
    HSAMPLE & Random & Dictionary & \(k\) \\
    Floyd's F2 & none & Set & \(k\) \\
    Our work & Sorted & none & \(k \log k\)
    \end{tabular}
\end{center}
...
Iterate in order through each candidate to add to the list in order,
...
\subsection{SAMPLE}
\inputminted{Python}{code/fisheryates.py}
Use a Fisher-Yates shuffle to randomize only the first \(k\) items
of an array of the elements of \(\NN_{<n}\).

\subsection{HSAMPLE}
\inputminted{Python}{code/selby_fy.py}
As above, except that instead of allocating an initializing an \(n\)-element array,
